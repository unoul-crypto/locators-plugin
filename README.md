...
Готовый файл: `target/Locators-1.2.1.jar`. Поместите его в `plugins` сервера и перезапустите сервер.
...
Значения yaw и pitch в результатах подчёркнуты и кликабельны. При нажатии плагин поворачивает камеру
на показанный угол, не меняя позицию игрока. Yaw меняет только горизонтальный поворот, pitch — только
вертикальный; при необходимости можно последовательно нажать оба значения. Каждую ссылку можно
использовать многократно в течение 120 секунд; она привязана к получившему результат игроку.
